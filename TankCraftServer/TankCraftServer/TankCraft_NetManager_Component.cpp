...
uint Xn::TankCraft::NetManager_Component::ConnectToServer(std::wstring ipV4,
  ...
  mClientThread = new std::thread(TcpUtils::ClientThreadFunction,
                                  std::string(ipBuf), iPort, this);
...
void Xn::TankCraft::NetManager_Component::
  ...
  if (HasClientRequest()) {
    ...
    while (!from_client_datas_buffers_[from_client_datas_buffer_index_]
                .isEmpty()) {
...
void Xn::TankCraft::NetMessageBaseData::SetData(wchar_t* nData, int nLen) {
  FreeData();
  length = nLen;
...
void Xn::TankCraft::NetMessageBaseDataBuffer::Push(
    std::unique_ptr<NetMessageBaseData> data) {
  datas_.push(std::move(data));
}

bool Xn::TankCraft::NetMessageBaseDataBuffer::isEmpty() const {
  return datas_.empty();
}

std::unique_ptr<NetMessageBaseData>
Xn::TankCraft::NetMessageBaseDataBuffer::Pop() {
  if (isEmpty()) {
    return nullptr;
  } else {
...
void Xn::TankCraft::NetMessageBaseDataBuffer::Clear() {
  while (!isEmpty()) {
    Pop();
  }
}

NetManager_Component::NetManager_Component()
