...
class TcpUtils {
 public:
  ...
  static int SendTcpDataToSocket(const TcpData* tcpData,
                                 void* socketClient); /* 可能返回 -1 */

  /* 客户端接收数据的线程 */
  static void ClientThreadFunction(
      std::string ip, int port,
      Xn::TankCraft::NetManager_Component* nmComponent);

  /* 将多个 Request 消息打包成一个 Request 数据包 */
  static void CompactTcpDataListToTcpDataRequest(const TcpDataList* tcpDataList,
                                                 TcpData* tcpDataRequest);

  /* 将一个数据包拆分成多个 Message*/
  static void UnpackTcpDataMessageToTcpDataList(const TcpData* pTcpDataMessage,
                                                TcpDataList* pTcpDataList);

  /* 将 tcpDataList 转换成 nmBaseDataList */
  static void GetTcpDataListFromNetMessageBaseDataList(
