...
uint Xn::TankCraft::NetManager_Component::ConnectToServer(const uint16& ipV4_1, const uint16& ipV4_2, const uint16& ipV4_3, const uint16& ipV4_4, const uint& port)
{
	std::wstring wsIp =
		std::to_wstring(ipV4_1) + L"." +
		std::to_wstring(ipV4_2) + L"." +
		std::to_wstring(ipV4_3) + L"." +
...
uint Xn::TankCraft::NetManager_Component::ConnectToServer(std::wstring ipV4, std::wstring port)
{
	/* 将 wchar_t* 转化为 char* */
	char ipBuf[20] = {};
	int pos = 0;
	...
	mClientThread = new std::thread(TcpUtils::ClientThreadFunction, ipBuf, iPort, this);
...
NetMessageBaseDataBuffer* const& Xn::TankCraft::NetManager_Component::TryGetServerToClientMessageBuffer()
{
	Lock();
	
	/* from_server_datas_buffer_index_ 是当前的服务线程正在占用的 buffer index */
	NetMessageBaseDataBuffer* foo = &from_server_datas_buffers_[from_server_datas_buffer_index_];
...
NetMessageBaseDataBuffer* const& Xn::TankCraft::NetManager_Component::TryGetClientToServerMessageBuffer()
{
	Lock();
	
	/* from_client_datas_buffer_index_ 是当前的服务线程正在占用的 buffer index */
	NetMessageBaseDataBuffer* bufferFrom = &from_client_datas_buffers_[from_client_datas_buffer_index_ ^ 1];
	NetMessageBaseDataBuffer* bufferTo = &from_client_datas_buffers_[from_client_datas_buffer_index_];
...
{
	/* 新申请一个数据缓存 */
	std::unique_ptr<Xn::TankCraft::NetMessageBaseData> newMsg(new Xn::TankCraft::NetMessageBaseData);
...
void Xn::TankCraft::NetManager_Component::PushToFromServerList(std::unique_ptr<NetMessageBaseData> nmData)
{
	/* 将数据移动到对应的队列中 */
	Lock();
	from_server_datas_buffers_[from_server_datas_buffer_index_].Push(std::move(nmData));
	Unlock();
}

void Xn::TankCraft::NetManager_Component::Lock() const
{
	mMyMutex.lock();
}

void Xn::TankCraft::NetManager_Component::Unlock() const
{
	mMyMutex.unlock();
}
